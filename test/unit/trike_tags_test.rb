...
class TrikeTagsTest < Test::Unit::TestCase
	...
	def test_that_img_renders_a_helpful_error_if_host_not_found
		@page.stubs(:host).raises(StandardTags::TagError, "`host' tag error message")

		begin
			@parser.parse('<r:img src="/dir/img.jpg" attr="arbitrary" />')
		rescue StandardTags::TagError => e
			assert e.message.match(/`img' tag/), "tag error doesn't mention `img' tag - #{e.message}"
		end
	end
